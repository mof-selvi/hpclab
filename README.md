...
- If you need to run a .py file directly on Terminal, connect to one of the nodes:
  ...
  ```
  srun -p longer --time=14-0 -N1 -n1 --gres=gpu:1 --job-name=job01 --nodelist=ne01,ne02,ne03 --pty bash
  ```
  ...
  ```
  srun -p long --time=7-0 -N1 -n1 --gres=gpu:1 --job-name=auj01 --pty bash
  ```

  ```
  srun -p longer --time=15-0 -N1 -n1 --gres=gpu:1 --job-name=auj01 --pty bash
  ```
